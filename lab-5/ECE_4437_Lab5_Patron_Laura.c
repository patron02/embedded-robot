...
void ADCSeq3IntHandler (void) {
  ...
  //  UARTSend(UART0_BASE, (uint8_t *)"ADC Handler: ",13);
    ADCSequenceDataGet(ADC0_BASE, 3, &ADCAvVal);
    volts = ADCAvVal * (3.3/4095);

    //distance = 13*pow(volts, -1)
  //  distance = 13.68/volts;

    //PF1 Red, PF2 Blue, PF3 Green
    if (volts > 1.1){ //red, less than 6 cm
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3, GPIO_PIN_1);
    }
    else if (volts > 0.8){ //yellow, between 6 and 10 cm
        GPIOPinWrite(GPIO_PORTF_BASE, GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3, GPIO_PIN_1 | GPIO_PIN_3);
    }
    else if (volts >= 0){ //green, greater than 10 cm
